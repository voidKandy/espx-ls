...
use crate::{config::GLOBAL_CONFIG, state::espx::AgentID, state::GlobalState};
use espionox::agents::memory::MessageRole;
use lsp_types::{GotoDefinitionResponse, HoverContents, Range, TextEdit, Uri, WorkspaceEdit};
use rand::Rng;
...
impl EchoBurn {
    ...
    pub fn goto_conversation_file(&self) -> GotoDefinitionResponse {
        ...
        let uri = Uri::from_str(&path_str).expect("Failed to build LSP URL from tempfile path");
