...
impl Database {
    ...
    pub async fn get_burns_by_url(&self, url: &Uri) -> DatabaseResult<Vec<InBufferBurn>> {
    ...
    pub async fn get_info_by_url(&self, url: &Uri) -> DatabaseResult<Option<DBDocumentInfo>> {
        let query = format!(
    ...
    pub async fn get_chunks_by_url(&self, url: &Uri) -> DatabaseResult<ChunkVector> {
        let query = format!(
    ...
    pub async fn get_all_docs(&self) -> DatabaseResult<Vec<FullDBDocument>> {
    ...
    pub async fn update_doc_store(&self, text: &str, url: Uri) -> DatabaseResult<()> {
    ...
    pub async fn get_full_doc_by_url(&self, url: &Uri) -> DatabaseResult<Option<FullDBDocument>> {
    ...
    pub async fn insert_burn(&self, burn: &InBufferBurn) -> DatabaseResult<Record> {
        let mut burn_vec = self.client.create("burns").content(burn).await?;
        let r: Record = burn_vec.remove(0);
        Ok(r)
    }

    pub async fn insert_doc_info(&self, info: &DBDocumentInfo) -> DatabaseResult<Record> {
        let r = self
            .client
    ...
    pub async fn remove_burn_by_url(&self, url: &Uri) -> DatabaseResult<()> {
        let query = format!("DELETE {} WHERE url = $url", "burns");
        self.client.query(query).bind(("url", url)).await?;
        Ok(())
    }

    pub async fn remove_doc_by_url(&self, url: &Uri) -> DatabaseResult<Option<DBDocumentInfo>> {
        Ok(self
            .client
    ...
    pub async fn remove_chunks_by_url(&self, url: &Uri) -> DatabaseResult<()> {
        let query = format!(
